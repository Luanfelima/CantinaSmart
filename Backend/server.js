...
app.get('/produtos', authenticateToken, (req, res) => {
  ...
  const query = `
    SELECT p.*, p.quantidade_produto, p.nivel_critico
    FROM produtos p
...
app.post('/produtos', authenticateToken, (req, res) => {
  const matricula_gestor = req.gestor.matricula_gestor;
  const { nome_p, categoria, preco, perecivel, descricao, unidade_medida, quantidade_produto, nivel_critico } = req.body;

  console.info(`Gestor Matrícula: ${matricula_gestor} - Criando novo produto`, req.body);

  const query = `
    INSERT INTO produtos (nome_p, categoria, preco, perecivel, descricao, unidade_medida, quantidade_produto, nivel_critico) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `;
  const values = [nome_p, categoria, preco, perecivel, descricao, unidade_medida, quantidade_produto, nivel_critico];
  ...
  db.query(query, values, (err, results) => {
    ...
    db.query(query_pg, [id_produto, matricula_gestor], (err) => {
      ...
      res.status(201).json({ id_produto, nome_p, categoria, preco, perecivel, descricao, unidade_medida, quantidade_produto, nivel_critico });
    });
  });
});
...
app.put('/produtos/:id_produto', authenticateToken, authorizeProduto, (req, res) => {
  ...
  const query = `
    UPDATE produtos 
    SET nome_p = ?, categoria = ?, preco = ?, perecivel = ?, descricao = ?, unidade_medida = ?, quantidade_produto = ?, nivel_critico = ?
    WHERE id_produto = ?
  `;
  const values = [nome_p, categoria, preco, perecivel, descricao, unidade_medida, quantidade_produto, nivel_critico, id_produto];
